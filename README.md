...
* VS코드에서는 **Git: Auto Fetch (all)** 를 켜면 주기적으로 자동으로 fetch됨.
---
### 2️. 로컬 브랜치 생성
```bash
...
* 격 브랜치의 최신 상태를 기준으로 작업할 수 있음
* 주의: `git checkout origin/<브랜치>`만 하면 detached HEAD 상태가 됨
---
### 3️. 작업 후 커밋
```bash
git add .
...
```
* 변경사항을 스테이징하고 로컬에 커밋
* 커밋은 로컬에서만 존재하며, 팀원에게는 아직 보이지 않음
---
...
* 다른 팀원의 변경사항은 **pull** 또는 **VS코드 Auto Fetch + pull**을 통해 로컬에 반영
---
## [4] 구현 내용

### 1. 기능 구현
- **3번 키**: 왼손 OK 제스처 미구현 상태에서 기능 구현 완료.  
- **4번 키**: 각 손의 Call 제스처 구현 완료.  
- **5번 키**: 두 손을 사용한 Olise 제스처 구현 완료.  

### 2. UI 개선
- 전체화면 모드 지원 가능한 창 형태로 수정.
---
